...
{
  for file in README.md CHANGELOG.md CONTRIBUTING_PLUGIN.md; do
    if [[ -f "${file}" ]]; then
      printf '%s\n' "${file}"
    fi
  done
...
while IFS= read -r token; do
  ...
  if [[ ! "${candidate}" =~ ^(\.\./)?(docs|tests|src|scripts|examples|plugin_template|plugins-index|\.github|README\.md|CHANGELOG\.md|CONTRIBUTING_PLUGIN\.md|LICENSE)(/.*)?$ ]]; then
    continue
  fi

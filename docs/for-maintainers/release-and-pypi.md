...
3. Run one lineage-writing command (`run`, `report-quality`, `adapter infer`, `eda`, `reliability check`, `ingest sync`, `ingest validate`, or `ingest sync-all`) and verify `.honestroles/runs/<run_id>/run.json` is created.
